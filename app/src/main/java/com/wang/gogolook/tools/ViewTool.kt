...
object ViewTool {
    ...
    /**
     * 從View產生Bitmap (指定長，寬以Unspecific計算)<br></br>
     * [重要]呼叫者有義務將回傳的Bitmap在使用完畢後recycle掉
     *
     * @return null 或 [Bitmap.Config.RGB_565]格式的Bitmap
     */
    fun createBitmapWithExactlyHeight(v: View?, exactlyHeight: Int): Bitmap? {
        if (null == v) {
            return null
        }
        ...
        if (v.width + v.height <= 0) {
            v.measure(
                ...
                View.MeasureSpec.makeMeasureSpec(exactlyHeight, View.MeasureSpec.EXACTLY)
            )
            v.layout(0, 0, v.measuredWidth, v.measuredHeight)
        }
        try {
            val bm = Bitmap.createBitmap(v.width, v.height, Bitmap.Config.RGB_565)
